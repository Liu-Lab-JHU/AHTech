...
and the observed data (y_test) in the .csv file. LCE is used as the
label in this example.

Please make sure Python3 as well as corresponding libraries (Pandas,
...
def make_plot(y_test, y_pred, rsme, r2_score, mae):
    ...
    r2_patch = mpatches.Patch(
        label="R$^2$ = {:.2f}".format(r2_score),
        color="#1f77b4"
    )
    rmse_patch = mpatches.Patch(
        label="RMSE = {:.2f}".format(rmse),
        color="#1f77b4"
    )
    mae_patch = mpatches.Patch(
        label="MAE = {:.2f}".format(mae),
        color="#1f77b4"
    )
    plt.xlim(0, 4)
    plt.ylim(0, 4)
